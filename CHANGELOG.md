...
### 修复
- **评论回显跨页串台（访客 cookie 回显机制漏洞）**：访客提交评论后 cookie 会记录自己发过的评论 id（30 天），用于前台显示「待审核」徽标；但回显 SQL 的 `OR id IN (...)` 分支缺少文章/项目约束（SQL 中 AND 优先级高于 OR，`(post_id=? AND ...) OR id IN (...)` 后半段不受 post_id 限制）——在文章 A 提交的待审核评论会串台显示到文章 B / 项目页评论区。修复：OR 分支补 `AND post_id=?`（项目页同理）圈定范围
- **被拒评论顶着「待审核」徽标误导**：回显不区分 pending/rejected，被拒绝的评论在 30 天 cookie 有效期内持续显示「待审核」——修复：回显分支排除 `rejected` 状态
- 管理员视角与后台列表不受影响；昵称校验防伪造逻辑保留

## [v1.3.57] - 2026-09-21

### 修复
- **移动端正文字距发「散」**：根因是两端对齐（`text-align: justify`）对含短行的段落自动拉伸字距补齐右边缘——移动端（≤767px）正文与列表项的 justify 规则整体移除，恢复左对齐；同块正文字间距 `letter-spacing: 0.03em` 一并归零（16px 下约 0.5px/字，叠加观感更散）。移动端排版现为 16px / 1.6 行高 / 24px 段距 / 无字间距 / 左对齐；桌面端 0.02em（视觉约 0.31px，无感）保留不动
