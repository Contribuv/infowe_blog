...
- `app.py` VERSION 定格 **1.3.12**（此后小改动不再逐版本递增，changelog 合并记录）
...
### v1.3.12 后续合并（2026-09-06，版本号不再递增）

- **评论 IP 归属地**：`_ip_location` 增加第三数据源 `ip-api.com`（前两源超时时兜底）；本机 `127.*` 直接返回空不显示
- **后台评论管理「来源」列**：展示 `ip_location`（badge）+ `ip_text`（子行），两者皆空显 `—`
- **邮件发件名称**：settings 新键 `smtp_sender_name`（空回退博客名），`_smtp_send` 的 `From` 头使用该名称
- **邮件模板样式修复**：CSS 原先被拼进 `<div style="...">` 属性导致选择器规则全部失效，改为完整 HTML 文档 + `<head><style>` 内嵌（评论通知邮件 + 两处 SMTP 测试邮件）
- **博主评论免审核**：评论者邮箱 == 后台「收件人」邮箱时直接 `approved`，无需审核；其余仍 `pending`
- **后台项目表格**：表格外包 `.table-wrap` 支持横向滚动；语言列只显示前 3 个 + `+N` 折叠徽标（占比移入悬浮提示），列宽重排（项目名 20% / 描述 26% / 语言 18%）；移动端隐藏排序列、描述完整换行、长仓库地址断行
